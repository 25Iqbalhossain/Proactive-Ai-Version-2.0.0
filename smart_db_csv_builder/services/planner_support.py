...
def _build_projected_column_index(
    ...
    return projected_counter, canonical_output_names
...
def _query_role_summary(columns: list[str]) -> dict[str, bool]:
    ...
    for column in columns:
        if not summary["has_user"] and _column_matches(column, USER_COLUMN_HINTS):
            summary["has_user"] = True
        if not summary["has_item"] and _column_matches(column, ITEM_COLUMN_HINTS):
            summary["has_item"] = True
        if not summary["has_rating"] and _column_matches(column, RATING_COLUMN_HINTS):
            summary["has_rating"] = True
        if not summary["has_time"] and _column_matches(column, TIME_COLUMN_HINTS):
            summary["has_time"] = True
        if not summary["has_metadata"] and _column_matches(column, METADATA_COLUMN_HINTS):
...
def _final_columns_are_usable(
    ...
    summary = _query_role_summary(final_columns)

    if rec_type in (RecSystemType.COLLABORATIVE, RecSystemType.HYBRID, RecSystemType.SEQUENTIAL):
        return summary["has_user"] and summary["has_item"]
    if rec_type == RecSystemType.CONTENT_BASED:
        return summary["has_item"] and summary["has_metadata"]
...
def _sanitize_plan(plan_dict: dict, schemas: list[SchemaResponse], rec_type: RecSystemType) -> dict:
    ...
    projected_counter, canonical_output_names = _build_projected_column_index(projected_columns_per_query)
...
def _analyze_table_columns(column_names: tuple[str, ...]) -> dict[str, object]:
    features = {
        "user": [],
        "item": [],
        "interaction": [],
    ...
    for name in column_names:
        ...
        if _column_matches(name, RATING_COLUMN_HINTS):
            features["rating"].append(name)

    def best(candidates: list[str], hints: tuple[str, ...]) -> str | None:
        if not candidates:
            return None
        return max(candidates, key=lambda name: _column_preference_score(name, hints))

    metadata_ranked = sorted(
        ((_column_preference_score(name, METADATA_COLUMN_HINTS), name) for name in features["metadata"]),
        reverse=True,
    )
    metadata_cols: list[str] = []
    seen_metadata: set[str] = set()
    for _, name in metadata_ranked:
    ...
    user_col = best(features["user"], USER_COLUMN_HINTS)
    item_col = best(features["item"], ITEM_COLUMN_HINTS)
    rating_col = best(features["rating"], RATING_COLUMN_HINTS)
    time_col = best(features["time"], TIME_COLUMN_HINTS)
    interaction_cols = tuple(
        name for name in features["interaction"] if name not in {rating_col, time_col}
    )

    return {
        "features": {
            "user": tuple(features["user"]),
            "item": tuple(features["item"]),
            "interaction": tuple(features["interaction"]),
            "time": tuple(features["time"]),
            "metadata": tuple(features["metadata"]),
        },
        "roles": {
            "user_col": user_col,
            "item_col": item_col,
...
def _describe_table_roles(table) -> dict[str, object]:
    roles = _analyze_table_columns(_table_column_names(table))["roles"]
    return {
        "user_col": roles["user_col"],
        "item_col": roles["item_col"],
        "rating_col": roles["rating_col"],
        "time_col": roles["time_col"],
        "metadata_cols": list(roles["metadata_cols"]),
        "interaction_cols": list(roles["interaction_cols"]),
    }
...
def _collect_table_features(table) -> dict[str, list[str]]:
    features = _analyze_table_columns(_table_column_names(table))["features"]
    return {bucket: list(values) for bucket, values in features.items()}
...
def _find_join_pair(
    ...
    role_lookup = role_lookup or (lambda table_obj: _describe_table_roles_with_schema(schema, table_obj, rec_type))
    primary_roles = role_lookup(primary_table)
    other_roles = role_lookup(other_table)
    ...
    if primary_key and other_key and primary_key.lower() == other_key.lower():
...
def _build_fallback_plan(
    ...
    role_cache: dict[tuple[str, str], dict[str, object]] = {}
    score_cache: dict[tuple[str, str], float] = {}
    rank_cache: dict[tuple[str, str], tuple[float, float]] = {}

    def table_key(schema: SchemaResponse, table) -> tuple[str, str]:
        return schema.connection_id, table.full_name.lower()

    def get_roles(schema: SchemaResponse, table) -> dict[str, object]:
        key = table_key(schema, table)
        if key not in role_cache:
            role_cache[key] = _describe_table_roles_with_schema(schema, table, rec_type)
        return role_cache[key]

    def get_score(schema: SchemaResponse, table) -> float:
        key = table_key(schema, table)
        if key not in score_cache:
            score_cache[key] = _score_table_for_rec_type(table, rec_type, target_description, schema=schema)
        return score_cache[key]

    def get_rank(schema: SchemaResponse, table) -> tuple[float, float]:
        key = table_key(schema, table)
        if key not in rank_cache:
            score = get_score(schema, table)
            roles = get_roles(schema, table)
            bonus = 0.0
            ...
            rank_cache[key] = (score + bonus, score)
    ...
    table_pool.sort(
        key=lambda item: get_rank(item[0], item[1]),
        reverse=True,
    )
    primary_schema, primary_table, _ = table_pool[0]
    primary_roles = get_roles(primary_schema, primary_table)
    ...
    def add_related_table(role: str):
        ...
        for schema, other_table, _ in table_pool:
            ...
            other_roles = get_roles(schema, other_table)
            join_pair = _find_join_pair(
                primary_schema,
                primary_table,
                other_table,
                role,
                rec_type,
                role_lookup=lambda table_obj: get_roles(primary_schema, table_obj),
            )
            if not join_pair:
                continue
            ...
            related_score = get_score(schema, other_table)
